...
const API_KEY = 'app_oupWMoNmY4CuMWxKzdzpluD30kjjJ65O'

export const renProfileAPI = {
  async sendMessage(messages) {
    try {
      ...
      const fullURL = `${API_BASE_URL}/api/apps/renprofile/chat/completions`
      
      console.log('=== Ren Profile API请求 ===')
      ...
      console.log('=== Ren Profile API响应成功 ===')
      console.log('响应数据:', response.data)
      return response.data
    } catch (error) {
      console.error('Ren Profile API调用错误 - 完整信息:', {
  ...
  async sendCustomRequest(requestBody) {
    try {
      ...
      console.log('=== Ren Profile 自定义API请求 ===')
      console.log('当前域名:', window.location.hostname)
      console.log('环境:', isDevelopment ? '开发环境' : '生产环境')
      console.log('API Base URL:', API_BASE_URL)
      console.log('完整URL:', fullURL)
      ...
      console.log('=== Ren Profile 自定义API响应成功 ===')
      console.log('响应数据:', response.data)
      return response.data
    } catch (error) {
      console.error('Ren Profile 自定义API调用错误 - 完整信息:', {
        status: error.response?.status,
        statusText: error.response?.statusText,
        data: error.response?.data,
        detail: error.response?.data?.detail
