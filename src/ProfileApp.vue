        ...
        if (firstChoice.message) {
          ...
          if (firstChoice.message.content && firstChoice.message.content.trim()) {
            ...
            if (isIncompleteResponse) {
              ...
              // 重新发送请求，强制要求生成完整答案
              try {
                const retryResponse = await fetch('http://113.106.62.42:9300/api/apps/renprofile/chat/completions', {
                  method: 'POST',
                  headers: {
            ...
            if (suggestedQuestions.length > 0) {
              ...
              // 发送确认请求获取答案
              try {
                const confirmResponse = await fetch('http://113.106.62.42:9300/api/apps/renprofile/chat/completions', {
                  method: 'POST',
                  headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer app_oupWMoNmY4CuMWxKzdzpluD30kjjJ65O'
